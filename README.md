...
> **⚠️ UIの注意点**  
> Cloudflare のダッシュボードは **Workers と Pages が同じ画面に統合** されています。  
> 左メニューの **「コンピュート（Workers）」→「Workers & Pages」** を開き、  
> **「アプリケーションを作成する」** をクリックしてください。  
>  
> すると最初は「Workers を作成」画面が表示されますが、**その画面の下のほうに**  
> **「Pages を導入しようとお考えですか？ 始める」** という小さなリンクがあります。  
> 見逃しやすいので注意してください。そこをクリックすると Pages の作成画面に進めます。

1. 上記の手順で Pages の作成画面を開く
